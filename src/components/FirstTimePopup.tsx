...
const OFFERS = [
  ...
  },
  {
    ...
    title: "Book a Table 🪑",
    description:
      "Reserve your table in advance and enjoy a special welcome treat.",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
      ...
      <div className="relative z-50 max-w-2xl w-full bg-card rounded-2xl shadow-2xl overflow-hidden animate-in fade-in scale-in-95 duration-300 max-h-[90vh] flex flex-col">
              ...
              <div className="p-5 sm:p-6 md:p-8 flex flex-col justify-between w-full md:w-1/2 text-foreground">
                ...
                <div className="space-y-3 mt-4">
                  ...
                  {!item.enable_online_order && !item.enable_reservation && (
                    <Button
                      className="w-full py-2.5 sm:py-3 bg-muted hover:bg-accent text-foreground font-semibold rounded-lg flex items-center justify-center gap-2 transition-all"
                      onClick={() => setOpen(false)}
                    >
                      {item.cta}
                    </Button>
        ...
        <button
          ...
          className="absolute left-2 top-1/2 -translate-y-1/2 bg-card border rounded-full p-1 shadow z-10 cursor-pointer"
        >
        ...
        <button
          aria-label="next"
          onClick={() =>
            scrollToIndex(Math.min(OFFERS.length - 1, current + 1))
          }
          className="absolute right-2 top-1/2 -translate-y-1/2 bg-card border rounded-full p-1 shadow z-10 cursor-pointer"
        >
